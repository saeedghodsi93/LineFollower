...
int32_t USB_FileOpen (uint8_t counter)
{
    int32_t rc;
    FILE_ENTRY  *entry = 0;
    int32_t  fd = 0;
	...
	rc = USB_FindEntry(counter, entry);
    if (rc == MATCH_FOUND) {
        entry->FileStatus = 1;
        rc = fd+1;
    }
...
int32_t USB_FindEntry (uint8_t counter, FILE_ENTRY *entry)
{
              uint32_t  sec_num;
    volatile  uint8_t  *buf;
              uint8_t   ent_type;
    ...
    for (sec_num = FAT_BootSec.RootDirStartSec;              /* For all the sectors in root directory       */
        ...
        while (buf < (FATBuffer + FAT_BootSec.BytsPerSec)) {
            ...
            if (ent_type == SFN_ENTRY) {                     /* If it is short entry get short file name    */
				...
				FAT_GetSfnExt(buf, temp_format);
				if(FAT_StrCaseCmp(temp_format, "wav") == MATCH_FOUND) {
	                                   								 /* Compare given name with this name case insensitively */
	...
	for (sec_num = FAT_BootSec.RootDirStartSec;              /* For all the sectors in root directory       */
        ...
        while (buf < (FATBuffer + FAT_BootSec.BytsPerSec)) {
            ...
            if (ent_type == SFN_ENTRY) {                     /* If it is short entry get short file name    */
				...
				UART0_SendString(ent_name_read);
				UART0_SendString("\n\r");
